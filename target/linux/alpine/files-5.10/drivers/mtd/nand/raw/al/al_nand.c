...
/**************************** configuration ***********************************/
/******************************************************************************/
static enum al_nand_ecc_bch_num_corr_bits bch_num_bits_convert(unsigned int bits)
{
	switch (bits) {
	case 4:
...
static enum al_nand_device_page_size page_size_bytes_convert(unsigned int bytes)
{
	switch (bytes) {
	case 2048:
...
static int al_nand_attach_chip(struct nand_chip *nand)
{
	...
	ret = al_nand_dev_config(&nand_dat->nand_obj,
				 ...
				 &nand_dat->ecc_config);
	if (ret) {
		dev_info(&nand_dat->pdev->dev, "al_nand_dev_config failed with %d\n", ret);
		return -EIO;
	}
