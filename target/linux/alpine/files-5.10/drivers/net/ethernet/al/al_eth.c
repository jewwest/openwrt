...
static const struct pci_device_id al_eth_pci_tbl[] = {
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS, PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH,
	  PCI_ANY_ID, PCI_ANY_ID, 0, 0, ALPINE_INTEGRATED},
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS, PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_ADVANCED,
	  PCI_ANY_ID, PCI_ANY_ID, 0, 0, ALPINE_INTEGRATED},
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS, PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_NIC,
	  PCI_ANY_ID, PCI_ANY_ID, 0, 0, ALPINE_NIC},
	{ PCI_VENDOR_ID_AMAZON_ANNAPURNA_LABS, PCI_DEVICE_ID_AMAZON_ANNAPURNA_LABS_ETH_FPGA_NIC,
...
static int al_eth_phy_init(struct al_eth_adapter *adapter)
{
	...
	ethtool_convert_link_mode_to_legacy_u32(&features,
						PHY_GBIT_FEATURES);
	...
	ethtool_convert_legacy_u32_to_link_mode(phydev->supported,
						supported);
...
static int
al_eth_get_link_ksettings(struct net_device *netdev, struct ethtool_link_ksettings *ecmd)
{
	struct al_eth_adapter *adapter = netdev_priv(netdev);
	struct phy_device *phydev = adapter->phydev;
...
static int
al_eth_set_link_ksettings(struct net_device *netdev, const struct ethtool_link_ksettings *ecmd)
{
	struct al_eth_adapter *adapter = netdev_priv(netdev);
	struct phy_device *phydev = adapter->phydev;
	int rc = 0;
