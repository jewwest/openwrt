 ...
 * updated).
 */
int ecc_read_page(struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
//	int bytes = mtd_ooblayout_count_eccbytes(mtd);
	...
	chip->legacy.cmdfunc(chip, NAND_CMD_READOOB, s_ecc_loc, page);
...
int ecc_read_subpage(struct nand_chip *chip, uint32_t offs, uint32_t len, uint8_t *buf, int page)
{
 ...
 */
int ecc_write_page(struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
	int bytes =s_oob_size - s_ecc_loc;
	uint32_t cmd;
	struct mtd_info *mtd;
		...
		struct al_nand_ecc_config *ecc_config)
{
	...
	for (i = AL_NAND_MAX_ONFI_TIMING_MODE ; i >= 0 ; i--) {
		if (BIT(i) & le16_to_cpu(nand->parameters.onfi->async_timing_mode)) {
			/*
			 * Addressing RMN: 2903
			 */
	...
	device_properties->num_col_cyc =
		((nand->parameters.onfi->addr_cycles & ONFI_COL_ADDR_CYCLE_MASK)
						>> ONFI_COL_ADDR_CYCLE_POS);
	device_properties->num_row_cyc =
		((nand->parameters.onfi->addr_cycles & ONFI_ROW_ADDR_CYCLE_MASK)
...
#ifdef AL_NAND_ECC_SUPPORT
	if (nand->base.eccreq.strength == 1) {
...
//	mtd->priv = nand;
	...
	ret = nand_scan_with_ids(nand, AL_NAND_MAX_CHIPS, NULL);
	if (ret) {
		pr_err("%s: nand_scan_with_ids failed\n", __func__);
		goto err;
	}
